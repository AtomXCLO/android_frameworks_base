...
#include <vendor/atomx/power/Feature.h>
#include <vendor/atomx/power/IPowerFeature.h>
#include "jni.h"
...
using PowerFeature = vendor::atomx::power::Feature;
using vendor::atomx::power::IPowerFeature;
...
static sp<IPowerFeature> gPowerAtomXHalAidl_ = nullptr;
...
static bool connectPowerFeatureHalLocked() {
    static bool gPowerAtomXHalAidlExists = true;
    if (gPowerAtomXHalAidlExists) {
        if (!gPowerAtomXHalAidl_) {
            gPowerAtomXHalAidl_ = waitForVintfService<IPowerFeature>();
        }
        if (gPowerAtomXHalAidl_) {
            ...
            gPowerAtomXHalAidlExists = false;
        }
    }
    return false;
}
...
static void nativeSetAtomXFeature(JNIEnv* /* env */, jclass /* clazz */, jint featureId,
                                   jint data) {
    // Try using AtomXs power hal. If it fails or isn't available, ignore
    if (connectPowerFeatureHalLocked()) {
        gPowerAtomXHalAidl_->setFeature(static_cast<PowerFeature>(featureId), static_cast<bool>(data));
    }
}
...
static const JNINativeMethod gPowerManagerServiceMethods[] = {
        ...
        {"nativeSetAtomXFeature", "(II)V", (void*)nativeSetAtomXFeature},
};
